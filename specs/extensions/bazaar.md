...
### Facilitator Response Header

After processing a `PaymentPayload`, a facilitator **MAY** append an `EXTENSION-RESPONSES` HTTP header to **both verify and settle responses** to communicate extension-specific outcomes to the client.

**Note:** This header is typically used during the **settle** phase to report cataloging results, but may also be included in **verify** responses for early validation feedback (e.g., schema validation failures detected during verification).
